# Daily Report — How to use it

A short procedure for running the daily payroll report on
...
1. From the dashboard, click **Daily report**.
...
`gazebo_daily_report_20260501-1530.xlsx`.

## 6. Run again

To process a new day, simply upload two new files and click
...
| **Contract match** = **Review** | The contract file has **conflicting** data for that Pay ID (duplicate IDs with different contract hours), or the match is ambiguous. Check the yellow warning banner and use **Show contract issues** in the table. Re-export **Employee Details (Advanced)** from ClockRite and contact the Developer if the conflict remains. |
| Yellow **Contract hours need attention** banner | One or more employees have Contract match **No** or **Review**, or the contract file has duplicate Pay ID conflicts. Review highlighted rows before exporting. |
